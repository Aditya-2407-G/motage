...
export default function TimelineTrack({ showRuler = true }) {
    ...
    interact(".timeline-item").draggable({
      ...
      listeners: {
        ...
        move(event) {
          ...
          const currentX = Number.parseFloat(target.getAttribute("data-x")) || 0
          ...
          const totalDx = Number.parseFloat(target.getAttribute("data-x")) || 0
    ...
    interact(".timeline-item").resizable({
      ...
      listeners: {
        ...
        move(event) {
          ...
          const finalX = Number.parseFloat(target.dataset.x) || 0
  ...
  const getEffectBadgeColor = (effect) => {
    switch (effect) {
      ...
      case "slide-left":
        return "bg-green-500/80 hover:bg-green-600/80 dark:bg-green-600/80 dark:hover:bg-green-700/80"
      case "slide-right":
        return "bg-green-500/80 hover:bg-green-600/80 dark:bg-green-600/80 dark:hover:bg-green-700/80"
      case "zoom-in":
      case "zoom-out":
        return "bg-purple-500/80 hover:bg-purple-600/80 dark:bg-purple-600/80 dark:hover:bg-purple-700/80"
      case "blur":
        return "bg-orange-500/80 hover:bg-orange-600/80 dark:bg-orange-600/80 dark:hover:bg-orange-700/80"
      default:
        return "bg-slate-500/80 hover:bg-slate-600/80 dark:bg-gray-600/80 dark:hover:bg-gray-700/80"
    }
  }
  ...
  return (
    <div ref={trackRef} className="relative h-full" onClick={() => setSelectedItem(null)}>
      {/* Time indicators */}
      {showRuler && (
        <div className="absolute top-0 left-0 right-0 h-6 bg-slate-100 dark:bg-gray-800 border-b border-slate-200 dark:border-gray-700">
          {Array.from({ length: Math.ceil(state.duration / 1000) + 1 }).map((_, i) => (
            <React.Fragment key={i}>
              <div
                className="absolute text-xs text-slate-600 dark:text-gray-400 font-medium"
              ...
              {i < Math.ceil(state.duration / 1000) &&
                Array.from({ length: 9 }).map((_, j) => (
                  <div
                    key={`${i}-${j}`}
                    className="absolute w-px h-2 bg-slate-300 dark:bg-gray-700"
                    style={{
      ...
      {items.map((item) => (
        <div
          ...
          style={{
            ...
            border:
              item.id === selectedItem?.id ? "2px solid rgba(139, 92, 246, 0.5)" : "1px solid rgba(139, 92, 246, 0.3)",
            borderRadius: "6px",
            overflow: "hidden",
            touchAction: "none",
            cursor: "move",
            ...
            userSelect: "none",
            boxShadow:
              item.id === selectedItem?.id ? "0 4px 12px rgba(139, 92, 246, 0.15)" : "0 2px 6px rgba(0, 0, 0, 0.05)",
        ...
        >
          ...
          {item.id === selectedItem?.id && (
            <button
              ...
              className="absolute top-2 right-2 bg-red-500 hover:bg-red-600 text-white rounded-full p-1 z-20 shadow-sm transition-colors"
            >
              <svg
          ...
          {/* Resize handles */}
          <div className="absolute left-0 top-0 bottom-0 w-2 cursor-w-resize bg-violet-500/50 dark:bg-violet-600/50 opacity-0 group-hover:opacity-100 transition-opacity" />
          <div className="absolute right-0 top-0 bottom-0 w-2 cursor-e-resize bg-violet-500/50 dark:bg-violet-600/50 opacity-0 group-hover:opacity-100 transition-opacity" />
          ...
          <div className="h-full w-full relative">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent pt-4 pb-1 px-2">
              <div className="text-white text-xs flex items-center justify-between">
                <span className="font-medium">{(item.duration / 1000).toFixed(1)}s</span>
